...
// Compatibility shim for Cubism 5.3+ features not in Core 5.1
(function() {
  if (typeof Live2DCubismCore === 'undefined') return;

  // Add dummy Offscreens support
  var OriginalModel = Live2DCubismCore.Model;
  if (OriginalModel && OriginalModel.fromMoc) {
    var originalFromMoc = OriginalModel.fromMoc;
    OriginalModel.fromMoc = function(moc) {
      var model = originalFromMoc.call(this, moc);
      if (model && !model.offscreens) {
        model.offscreens = {
          count: 0,
      ...
      // Add missing drawables properties if needed
      if (model && model.drawables) {
        if (!model.drawables.multiplyColors) {
          model.drawables.multiplyColors = new Float32Array(model.drawables.count * 4).fill(1.0);
        }
        if (!model.drawables.screenColors) {
          model.drawables.screenColors = new Float32Array(model.drawables.count * 4).fill(0.0);
        }
        if (!model.drawables.parentPartIndices) {
          model.drawables.parentPartIndices = new Int32Array(model.drawables.count).fill(-1);
        }
        if (!model.drawables.blendModes) {
          model.drawables.blendModes = new Int32Array(model.drawables.count);
        }
      }
      // Add missing parts properties if needed
      if (model && model.parts) {
        if (!model.parts.offscreenIndices) {
  ...
  // Patch Version.csmGetMocVersion to accept single ArrayBuffer parameter
  if (Live2DCubismCore.Version) {
    var originalGetMocVersion = Live2DCubismCore.Version.csmGetMocVersion;
    Live2DCubismCore.Version.csmGetMocVersion = function(mocBytes) {
      if (arguments.length === 1 && mocBytes instanceof ArrayBuffer) {
        var moc = Live2DCubismCore.Moc.fromArrayBuffer(mocBytes);
        if (moc) {
          return originalGetMocVersion.call(this, moc, mocBytes);
        }
        return 0;
      }
      return originalGetMocVersion.apply(this, arguments);
    };
  }
})();
